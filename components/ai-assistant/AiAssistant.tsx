/**
 * @fileoverview YYC³ AI 智能助理浮窗组件 v3.0
 * @description Adapted from docs/AI-Dev/AIAssistant/AIAssistant.tsx — v3.0
 * 模块化重构 + 双模式 (floating/inline)
 * @version 1.0.0
 * @author YYC³ Team
 *
 * @dependencies:
...
export function AiAssistant({
  ...
  // ---- Panel state ----
  const [internalOpen, setInternalOpen] = useState(mode === 'inline' ? defaultOpen : false);
  ...
  const [localApiKey, setLocalApiKey] = useState(() => {
    try {
      return localStorage.getItem('yyc3_ai_assistant_api_key') ?? externalApiKey;
    } catch {
  ...
  const [localModel, setLocalModel] = useState(() => {
    try {
      return localStorage.getItem('yyc3_ai_assistant_model') ?? '';
    } catch {
      return '';
    }
  });
  const [localTemperature, setLocalTemperature] = useState(() => {
    try {
      return parseFloat(localStorage.getItem('yyc3_ai_assistant_temperature') ?? '0.7');
    } catch {
      return 0.7;
    }
  });
  const [localTopP, setLocalTopP] = useState(() => {
    try {
      return parseFloat(localStorage.getItem('yyc3_ai_assistant_top_p') ?? '0.9');
    } catch {
      return 0.9;
    }
  });
  const [localMaxTokens, setLocalMaxTokens] = useState(() => {
    try {
      return parseInt(localStorage.getItem('yyc3_ai_assistant_max_tokens') ?? '2048');
    } catch {
      return 2048;
    }
  });
  const persist = (key: string, value: string) => {
    try {
      localStorage.setItem(`yyc3_ai_assistant_${key}`, value);
    } catch {
  ...
  const panelProps = {
    ...
    sendMessage,
    activePersona,
    setActivePersona: switchPersona,
    cmdFilter,
    setCmdFilter,
    personaFilter,
    setPersonaFilter,
  ...
  const PersonaIcon = currentPersona?.icon ?? YYCLogo;
    ...
    return (
      ...
      >
        ...
        <div className="absolute bottom-full right-0 mb-2 px-3 py-1.5 rounded-lg bg-[rgba(8,25,55,0.95)] border border-[rgba(0,180,255,0.2)] opacity-0 group-hover:opacity-100 transition-opacity pointer-events-none whitespace-nowrap">
          <span className="text-[#00d4ff]" style={{ fontSize: '0.72rem' }}>
            {currentPersona?.shortName ?? 'YYC³'} 智能助理
          </span>
        </div>
      </button>
    );
        ...
        <div className="shrink-0 border-b border-[rgba(0,180,255,0.12)] bg-[rgba(0,40,80,0.2)]">
          <div className="flex items-center justify-between px-4 py-2.5">
            <div className="flex items-center gap-2.5">
              <div
                className="w-8 h-8 rounded-lg flex items-center justify-center shadow-[0_0_15px_rgba(0,180,255,0.3)] overflow-hidden"
                style={{
                  background: `linear-gradient(135deg, ${currentPersona?.color ?? '#00d4ff'}, ${currentPersona?.color ? `${currentPersona.color}88` : '#7b2ff7'})`,
                }}
              >
                {currentPersona ? (
                  <PersonaIcon className="text-white" size={18} />
                ) : (
                  <YYCLogo size={20} />
                )}
              </div>
              <div>
                <h3
                  className="text-[#e0f0ff] flex items-center gap-1.5"
                  style={{ fontSize: '0.85rem' }}
                >
                  {currentPersona?.shortName ?? 'YYC³'}
                  <span className="text-[rgba(0,212,255,0.3)]" style={{ fontSize: '0.55rem' }}>
                    {currentPersona?.enTitle ?? 'AI'}
                  </span>
                  {currentPersona?.mood && (
                    <span className="text-xs">{moodEmoji[currentPersona.mood] ?? '✨'}</span>
                  )}
                </h3>
              ...
              return (
                ...
                >
                  ...
                  <span
                    style={{
                      fontSize: '0.5rem',
                      color: isActive ? p.color : 'rgba(0,212,255,0.3)',
