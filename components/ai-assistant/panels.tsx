/**
 * @fileoverview YYC³ AI Assistant - 8 个 Tab 面板组件
 * @description Adapted from docs/AI-Dev/AIAssistant/panels.tsx — v3.0
 * @version 1.0.0
 * @author YYC³ Team
 */
import {
    ...
    <>
          ...
          return (
            <div key={msg.id} className={`flex ${isUser ? 'justify-end' : 'justify-start'}`}>
              <div
                ...
                style={
                  !isUser && !isSystem
                    ? { background: `${pColor}10`, border: `1px solid ${pColor}20` }
                    : {}
                }
              >
                {!isUser && !isSystem && (
                  <EmotionRipple color={pColor ?? '#00d4ff'} active={!!msg.emotion} />
                )}
                <div className="relative px-3.5 py-2.5" style={{ zIndex: 1 }}>
                  {!isUser && msg.personaId && (
                    <div className="flex items-center gap-1 mb-1">
                      <span style={{ fontSize: '0.6rem', color: pColor }}>
                        {PERSONAS_MAP[msg.personaId]?.shortName}
                      </span>
                    </div>
                  )}
                  <div
                    className={`whitespace-pre-wrap ${isUser ? 'text-[#e0f0ff]' : isSystem ? 'text-[#ffdd00]' : 'text-[#c0dcf0]'}`}
                  ...
                  {msg.role === 'assistant' && (
                    <button
                      onClick={() => p.copyToClipboard(msg.content, msg.id)}
      ...
      <div className="shrink-0 p-3 border-t border-[rgba(0,180,255,0.1)]">
        <div className="flex items-end gap-2">
          {voiceSupported && (
            <button
              ...
              className={`p-2.5 rounded-xl transition-all min-w-[40px] min-h-[40px] flex items-center justify-center ${
                isListening
                  ? 'bg-[rgba(255,60,60,0.2)] border border-[rgba(255,60,60,0.4)] animate-pulse'
                  : 'bg-[rgba(0,40,80,0.4)] border border-[rgba(0,180,255,0.15)] hover:border-[rgba(0,180,255,0.3)]'
              }`}
              title={isListening ? '点击停止' : '语音输入'}
            >
              {isListening ? (
                <MicOff className="w-4 h-4 text-[#ff6060]" />
              ) : (
                <Mic className="w-4 h-4 text-[rgba(0,212,255,0.5)]" />
              )}
            </button>
          )}
          <textarea
            ...
            placeholder={
              isListening ? '🎤 正在聆听...' : '输入指令... (Enter 发送, Shift+Enter 换行)'
            }
            rows={1}
          ...
          />
          <button
            onClick={() => p.sendMessage(p.inputValue)}
            disabled={!p.inputValue.trim() || p.isTyping}
...
export function CommandsPanel(p: PanelProps) {
  const executeCommand = (cmd: SystemCommand) => {
    p.setActiveTab('chat');
    if (cmd.persona) {
      p.setActivePersona(cmd.persona);
    }
    p.sendMessage(cmd.action);
  };
  const filteredCommands = SYSTEM_COMMANDS.filter((c) => {
    if (p.cmdFilter !== 'all' && c.category !== p.cmdFilter) {
      return false;
    }
    if (p.personaFilter !== 'all' && c.persona !== p.personaFilter) {
      return false;
    }
    return true;
  });
    ...
    <div className="flex-1 overflow-auto p-3">
      <div className="flex items-center gap-1 mb-2 flex-wrap">
        {cmdCategories.map((cat) => (
          <button
            key={cat.key}
            onClick={() => p.setCmdFilter(cat.key)}
      ...
      <div className="flex items-center gap-1 mb-3 flex-wrap">
        ...
        {[
          { id: 'all' as const, shortName: '全部' as const, color: 'rgba(0,212,255,0.4)' as const },
          ...FAMILY_PERSONAS.map((p) => ({ id: p.id, shortName: p.shortName, color: p.color })),
        ].map((f) => (
          <button
            key={f.id}
            onClick={() => p.setPersonaFilter(f.id)}
            className={`px-2 py-0.5 rounded transition-all ${p.personaFilter === f.id ? 'border' : 'border border-transparent hover:border'}`}
            style={{
              fontSize: '0.6rem',
              background: p.personaFilter === f.id ? `${f.color}18` : 'transparent',
              borderColor: p.personaFilter === f.id ? f.color : 'transparent',
              color: p.personaFilter === f.id ? f.color : 'rgba(0,212,255,0.3)',
            }}
          >
            {f.shortName}
          </button>
          ...
          return (
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <p
                  className="text-[#e0f0ff] group-hover:text-[#00d4ff] transition-colors"
                  style={{ fontSize: '0.8rem' }}
                >
                  {cmd.label}
                </p>
                <p className="text-[rgba(0,212,255,0.35)] truncate" style={{ fontSize: '0.68rem' }}>
                  {cmd.desc} {cmd.persona ? `· ${PERSONAS_MAP[cmd.persona]?.shortName}` : ''}
                </p>
              </div>
          ...
          >
            <div className="flex items-center gap-3">
              <div
                ...
                style={{ background: `${persona.color}22`, border: `1.5px solid ${persona.color}` }}
              >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-[rgba(0,212,255,0.3)]" style={{ fontSize: '0.55rem' }}>
                    {persona.enTitle}
                  </span>
            ...
            {persona.modelName && (
              <div
                className="mt-2 flex items-center gap-2 text-[rgba(0,212,255,0.25)]"
...
export function SkillsPanel(p: PanelProps) {
  ...
  const filteredSkills =
    p.skillFilter === 'all' ? p.skills : p.skills.filter((s) => s.category === p.skillFilter);
  return (
    <div className="flex-1 overflow-auto p-3">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[rgba(0,212,255,0.3)]" style={{ fontSize: '0.62rem' }}>
          {p.skills.filter((s) => s.active).length}/{p.skills.length} 启用
        </span>
      </div>
      <div className="flex items-center gap-1 mb-3 flex-wrap">
        <button
          onClick={() => p.setSkillFilter('all')}
      ...
      <div className="space-y-2">
        {filteredSkills.map((skill) => {
          ...
          return (
            <div
              key={skill.id}
              className="flex items-center gap-3 p-3 rounded-xl bg-[rgba(0,40,80,0.2)] border border-[rgba(0,180,255,0.08)]"
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  <span className="text-[#e0f0ff]" style={{ fontSize: '0.8rem' }}>
                    {skill.name}
                  </span>
                  <span
                    className="px-1.5 py-0.5 rounded"
                ...
                <p className="text-[rgba(0,212,255,0.35)] truncate" style={{ fontSize: '0.65rem' }}>
                  {skill.description}
                </p>
              </div>
              <button
              ...
              >
                {skill.active ? (
                  ...
                  <ToggleLeft className="w-5 h-5 text-[rgba(0,212,255,0.2)]" />
                )}
              </button>
            </div>
          );
      ...
      <div className="mt-3 p-2.5 rounded-xl bg-[rgba(0,255,136,0.04)] border border-[rgba(0,255,136,0.1)]">
        <p
          className="text-[rgba(0,255,136,0.4)] flex items-center gap-1"
          style={{ fontSize: '0.6rem' }}
        >
          ...
          {p.skills
            ...
            .map((s) => s.name)
            .join(' · ')}{' '}
          等 {p.skills.filter((s) => s.active).length} 项技能活跃中
        </p>
      </div>
    </div>
  );
...
export function PluginsPanel(p: PanelProps) {
  ...
  const filteredPlugins =
    p.pluginFilter === 'all' ? p.plugins : p.plugins.filter((pl) => pl.category === p.pluginFilter);
  return (
    <div className="flex-1 overflow-auto p-3">
      <div className="flex items-center justify-between mb-3">
        <h4 className="text-[#e0f0ff]" style={{ fontSize: '0.82rem' }}>
        ...
        <span className="text-[rgba(0,212,255,0.3)]" style={{ fontSize: '0.62rem' }}>
          {p.plugins.filter((pl) => pl.enabled).length}/{p.plugins.length} 启用
        </span>
      </div>
      <div className="flex items-center gap-1 mb-3 flex-wrap">
        {[
          { key: 'all', label: '全部' },
          ...pluginCategories.map((c) => ({ key: c, label: c })),
        ].map((cat) => (
          <button
            key={cat.key}
            onClick={() => p.setPluginFilter(cat.key)}
          ...
          return (
            ...
            >
              ...
              <button
                onClick={() =>
                  p.setPlugins((prev) =>
                    prev.map((pl) => (pl.id === plug.id ? { ...pl, enabled: !pl.enabled } : pl))
                  )
                }
                className="shrink-0"
              >
  ...
  return (
    <div className="flex-1 overflow-auto p-3">
      <div className="flex items-center justify-between mb-3">
        ...
        <span className="text-[rgba(0,212,255,0.3)]" style={{ fontSize: '0.62rem' }}>
          {p.modules.filter((m) => m.loaded).length}/{p.modules.length} 已加载
        </span>
      </div>
      <div className="space-y-2">
        {p.modules.map((mod) => {
          ...
          return (
            ...
            >
              ...
              <button
                onClick={() =>
                  p.setModules((prev) =>
                    prev.map((m) => (m.id === mod.id ? { ...m, loaded: !m.loaded } : m))
                  )
                }
      ...
      <div className="mt-3 grid grid-cols-2 gap-2">
        <div className="p-2.5 rounded-xl bg-[rgba(0,255,136,0.04)] border border-[rgba(0,255,136,0.1)]">
          <div
            className="flex items-center gap-1.5 text-[rgba(0,255,136,0.5)]"
            style={{ fontSize: '0.6rem' }}
          >
            <Heart className="w-3 h-3" /> 系统健康
          </div>
          <span className="text-[#00ff88]" style={{ fontSize: '1rem', fontFamily: 'monospace' }}>
            96%
          </span>
        </div>
        <div className="p-2.5 rounded-xl bg-[rgba(0,212,255,0.04)] border border-[rgba(0,212,255,0.1)]">
          <div
            className="flex items-center gap-1.5 text-[rgba(0,212,255,0.5)]"
            style={{ fontSize: '0.6rem' }}
          >
            <Music2 className="w-3 h-3" /> 音乐状态
          </div>
          <span className="text-[#e0f0ff]" style={{ fontSize: '0.7rem' }}>
            🎵 星际协奏曲
          </span>
        </div>
      </div>
          ...
          >
            <div className="flex items-center justify-between mb-1.5">
              <div className="flex items-center gap-2">
                ...
                <span
                  className="px-1.5 py-0.5 rounded bg-[rgba(0,212,255,0.06)] text-[rgba(0,212,255,0.4)]"
    ...
    <div className="flex-1 overflow-auto p-3 space-y-4">
      <div>
        ...
        <div className="relative">
          <input
            type={p.showApiKey ? 'text' : 'password'}
            value={p.localApiKey}
            onChange={(e) => {
              p.setLocalApiKey(e.target.value);
              p.persist('ai_assistant_api_key', e.target.value);
            }}
          ...
          />
          <button
            onClick={() => p.setShowApiKey(!p.showApiKey)}
      ...
      <div>
        <h4 className="text-[#e0f0ff] mb-2 flex items-center gap-2" style={{ fontSize: '0.82rem' }}>
          <Cpu className="w-4 h-4 text-[#00d4ff]" /> 模型选择
        </h4>
        {p.availableModels.length > 0 ? (
          <div className="space-y-1">
            {p.availableModels.map((model) => (
              <button
                key={model.id}
                onClick={() => {
                  p.setLocalModel(model.id);
                  p.persist('ai_assistant_model', model.id);
                }}
              ...
              >
                {model.isLocal && (
                  <span className="text-[#00ff88] shrink-0" style={{ fontSize: '0.58rem' }}>
                    本地
                  </span>
                )}
                <span className="truncate flex-1">{model.name}</span>
                <span
                  className="text-[rgba(0,212,255,0.25)] shrink-0"
          ...
          <p
            className="text-[rgba(0,212,255,0.25)] text-center py-3"
            style={{ fontSize: '0.72rem' }}
          >
            暂无可用模型
          </p>
      ...
      <div>
        <h4 className="text-[#e0f0ff] mb-2" style={{ fontSize: '0.78rem' }}>
          家人模型绑定
        </h4>
        {FAMILY_PERSONAS.slice(0, 4).map((per) => (
          <div
            key={per.id}
            className="flex items-center justify-between px-3 py-1.5 rounded-lg bg-[rgba(0,40,80,0.15)] border border-[rgba(0,180,255,0.06)]"
          >
            <span className="text-[rgba(0,212,255,0.5)]" style={{ fontSize: '0.65rem' }}>
              {per.shortName}
            </span>
            <span className="text-[#e0f0ff]" style={{ fontSize: '0.62rem' }}>
              {per.modelName ?? '默认'}
            </span>
          </div>
        ))}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          ...
          <span className="text-[#00d4ff]" style={{ fontSize: '0.8rem' }}>
            {p.localTemperature.toFixed(2)}
          </span>
        </div>
        <input
          type="range"
          ...
          max="2"
          step="0.05"
          value={p.localTemperature}
          onChange={(e) => {
            p.setLocalTemperature(Number(e.target.value));
            p.persist('ai_assistant_temperature', e.target.value);
          }}
      ...
      <div>
        ...
        <input
          ...
          value={p.localTopP}
          onChange={(e) => {
            p.setLocalTopP(Number(e.target.value));
            p.persist('ai_assistant_top_p', e.target.value);
          }}
          className="w-full accent-[#00d4ff]"
        />
      </div>
      ...
      <div>
        ...
        <input
          ...
          max="8192"
          step="256"
          value={p.localMaxTokens}
          onChange={(e) => {
            p.setLocalMaxTokens(Number(e.target.value));
            p.persist('ai_assistant_max_tokens', e.target.value);
          }}
          className="w-full accent-[#00d4ff]"
        />
      </div>

      <div className="p-3 rounded-xl bg-[rgba(0,180,255,0.04)] border border-[rgba(0,180,255,0.1)]">
        ...
        <p className="text-[rgba(0,212,255,0.3)] mt-1" style={{ fontSize: '0.62rem' }}>
          当前活跃家人:{' '}
          {FAMILY_PERSONAS.find((persona) => persona.id === p.activePersona)?.name ?? '未选择'}
        </p>
      </div>
    </div>
  );
