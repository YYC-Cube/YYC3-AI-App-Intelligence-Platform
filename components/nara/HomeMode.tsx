    ...
    >
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-slate-50/80 rounded-xl p-3 border border-slate-100 transition-all hover:shadow-md hover:bg-white group">
          ...
          <div className="text-lg font-bold text-emerald-600 group-hover:scale-105 transition-transform origin-left">
            32%
          </div>
        </div>
        <div className="bg-slate-50/80 rounded-xl p-3 border border-slate-100 transition-all hover:shadow-md hover:bg-white group">
          <div className="text-xs text-slate-500 mb-1 font-medium">{t.memory}</div>
          <div className="text-lg font-bold text-blue-600 group-hover:scale-105 transition-transform origin-left">
            4.2GB
          </div>
        </div>
        <div className="bg-slate-50/80 rounded-xl p-3 border border-slate-100 transition-all hover:shadow-md hover:bg-white group">
          <div className="text-xs text-slate-500 mb-1 font-medium">{t.tasks}</div>
          <div className="text-lg font-bold text-purple-600 group-hover:scale-105 transition-transform origin-left">
            27
          </div>
        </div>
        <div className="bg-slate-50/80 rounded-xl p-3 border border-slate-100 transition-all hover:shadow-md hover:bg-white group">
          <div className="text-xs text-slate-500 mb-1 font-medium">{t.uptime}</div>
          <div className="text-lg font-bold text-amber-500 group-hover:scale-105 transition-transform origin-left">
            99.8%
          </div>
        </div>
      </div>
            ...
            {sections.map((section, index) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl bg-gradient-to-br ${section.color} flex items-center justify-center flex-shrink-0 shadow-sm`}
                    >
                    ...
                    <div className="flex-1 text-left">
                      ...
                      {section.badge && (
                        <span
                          className={`text-[10px] font-semibold px-2 py-0.5 rounded-full mt-1.5 inline-block ${section.badgeColor}`}
                        >
  ...
  return (
    <div className="h-full flex bg-slate-50">
            ...
            {sections.map((section) => {
              ...
              return (
                ...
                >
                  ...
                  <div
                    className={`w-8 h-8 rounded-lg bg-gradient-to-br ${section.color} flex items-center justify-center flex-shrink-0 shadow-sm group-hover:scale-105 transition-transform`}
                  >
                  ...
                  <div className="flex-1 text-left overflow-hidden">
                    <div className="flex items-center gap-1.5">
                      ...
                      {section.hasControls && (
                        <Sliders
                          className={`w-3 h-3 flex-shrink-0 ${isActive ? 'text-slate-400' : 'text-slate-300'}`}
                        />
                      )}
                    </div>
                    {section.badge && (
                      <span
                        className={`text-[10px] px-1.5 py-0.5 rounded font-medium mt-1 inline-block ${section.badgeColor}`}
                      >
                        {t.badges[section.badge as keyof typeof t.badges]}
                      </span>
                    )}
      ...
      <div className="flex-1 overflow-y-auto bg-slate-50/50 p-6">
        <div className="max-w-5xl mx-auto">{renderDetailPanel()}</div>
      </div>
    </div>
  );
}
